...
class MangleEventLogger(commands.Cog):
	...
	async def on_message(self, msg: nextcord.Message):
		if msg.author.id != self.mangle.user.id and const.BOT_CHANNEL_ID == msg.channel.id and msg.type == nextcord.MessageType.default:
			await msg.delete()
		return
	...
	async def on_raw_message_delete(self, pl: nextcord.RawMessageDeleteEvent):
		...
		if cache is not None:
			if (cache.author.id == self.mangle.user.id):
				return
			embed = create_embed(title="Message deleted :", color=COLORS_DICT['red'],
			                     description=f"Message send by {cache.author.mention} in {cache.channel.mention} has been deleted.")
			embed.add_field(name="Message content :", value=f"```{cache.content} ```", inline=False)
			await self.mangle.LOG_CHANNEL.send(embed=embed)
		else:
			await self.mangle.LOG_CHANNEL.send(embed=create_embed(color=COLORS_DICT['red'], title="Message deleted :",
			                                                      description="Unable to fetch message data."))
	...
	async def on_raw_message_edit(self, pl: nextcord.RawMessageUpdateEvent):
		...
		if cache is not None:
			embed = create_embed(title="Message edited :", color=COLORS_DICT['yellow'],
			                     description=f"Message send by{cache.author.mention} in {cache.channel.mention} has been modified.")
			...
			await self.mangle.LOG_CHANNEL.send(
				embed=create_embed(color=COLORS_DICT['yellow'], title="Message modified :",
				                   description="Unable to fetch message data."))

	@commands.Cog.listener()
	async def on_application_command_error(self, ia: nextcord.Interaction, exception):
		return await self.mangle.LOG_CHANNEL.send(embed=create_embed(title="Application command error",
	...
	async def on_command_error(self, ctx: commands.Context, exception):
		return await self.mangle.LOG_CHANNEL.send(embed=create_embed(title="Command error",
