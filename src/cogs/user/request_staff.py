...
class RequestStaffModal(nextcord.ui.Modal):
    def __init__(self, title, channel: nextcord.TextChannel, category: nextcord.CategoryChannel, request_type: str, notify_role:nextcord.Role):
        super().__init__(title=title)
        self.channel = channel
        self.category = category
        self.request_type = request_type
        self.notify_role = notify_role
        self.description = nextcord.ui.TextInput(label="Short description of your request", placeholder="", style=nextcord.TextInputStyle.paragraph, required=True, min_length=1, max_length=3968)
    ...
    async def callback(self, ia: nextcord.Interaction):
        ...
        await new_text_channel.set_permissions(target=ia.user, overwrite=nextcord.PermissionOverwrite(view_channel=True))
        embed: nextcord.Embed = create_embed(title=f"{self.request_type} request by {ia.user.display_name}", description=self.description.value, icon=ia.user.display_avatar, fields=(("Follow up channel", f"{new_text_channel.mention}"),))
        ...
        match self.request_type:
            case "Club":
                ...
                await msg.edit(content=f"{self.notify_role.mention} new club request by {ia.user.mention}", embed=embed, view=view)
            case "Event":
                await new_text_channel.edit(name=f"🔸{ia.user.display_name}")
                embed.colour = COLORS_DICT['orange']
                await msg.edit(content=f"{self.notify_role.mention} new event request by {ia.user.mention}", embed=embed, view=view)
            case "Generic":
                await new_text_channel.edit(name=f"▫️{ia.user.display_name}")
                embed.colour = COLORS_DICT['white']
                await msg.edit(content=f"{self.notify_role.mention} new request by {ia.user.mention}", embed=embed, view=view)
        return await new_text_channel.send(content=f"{ia.user.mention}, your {self.request_type} request will be followed up here : \n\n{self.description.value}")

class MangleRequestStaff(commands.Cog):
    def __init__(self, mangle: Mangle):
        self.mangle = mangle

    @nextcord.slash_command(name="request_bde", description="Make a request to the BDE. Your request will be followed by the BDE staff")
    async def request_bde(self, ia: nextcord.Interaction, request_type: str = nextcord.SlashOption(required=True, choices=REQUEST_TYPES)):
        ...
        return await ia.response.send_modal(RequestStaffModal(title=f"{request_type} request", request_type=request_type, channel=request_channel, category=request_channel.category, notify_role=self.mangle.BDE_NOTIFY_ROLE))